...
vector<int> two_merge(const int A[],const int B[],int m,int n){
   ...
   // For Removing  duplicates in C manually 
    // vector<int> D;
    // D.push_back(C[0]);
...
int main(){
    ...
    int m,n;
    //Receiving inputs from input file
    inputFile>>m>>n;
    int *A = new int[m];
    int *B = new int[n];
    ...
    outputFile<<"Outputs of all  subparts of this question are  here itself"<<endl;
    // Generating output of first part
    int* one_C = one_merge(A,B,m,n);
    outputFile<<"a.The merge of A and B gives: "<<endl;
    for(int i=0;i<m+n;i++){
        outputFile<<one_C[i]<<" ";
    }
    outputFile<<endl;

    // Generating output ofsecond part
    vector<int> two_C = two_merge(A,B,m,n);
    outputFile<<"b.The sorted array C for A U B : "<<endl;
    for(const auto &i:two_C){
        outputFile<<i<<" ";
    }
    outputFile<<endl;

    // Generating  output of third part
    vector<int> three_C = three_merge(A,B,m,n);
    outputFile<<"c.The sorted array C for A intersection B : "<<endl;
    ...
    outputFile<<"The time to run the current program is: "<<time_taken<<" sec"<<endl;
